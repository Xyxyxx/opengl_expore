...
int main() {
	...
	// yellow shader
	unsigned int yelloFragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(yelloFragmentShader, 1, &fragmentShaderSourceYellow, NULL);
	glCompileShader(yelloFragmentShader);

	success = 0;
	infoLog[0] = 0;
	glGetShaderiv(yelloFragmentShader, GL_COMPILE_STATUS, &success);
	if (!success) {
		glGetShaderInfoLog(yelloFragmentShader, 512, NULL, infoLog);
	...
	glAttachShader(yellowShaderProgram, yelloFragmentShader);
	...
	glDeleteShader(yelloFragmentShader);
